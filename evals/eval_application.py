...
GOLDEN_PATH = "goldens/correctness_goldens_application.json"
...
def run(rag):
    ...
    correctness = GEval(
        ...
        # 1. If we give evaluation_steps on our own, then the CoT step would be skipped and only the specified evaluation_steps would be used. Doing this makes the evaluation more deterministic
        # 2. If we don't give evaluation_steps, then the CoT step would be used and the model would generate its own evaluation steps.
        # 3. The best practice is to give the criteria when building the eval pipeline and let the model generate its own evaluation steps. 
        # Later, when you get an idea of the evaluation steps and the steps that the pipeline is failing on, you can give the evaluation_steps to make the evaluation more deterministic.
              
        # We will use rubric to define the scoring criteria for correctness evaluation 
        rubric=[
