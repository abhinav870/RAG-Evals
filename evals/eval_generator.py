...
GOLDEN_PATH = "goldens/faithfulness_golden_dataset.json"
...
def run():
    ...
    for g in goldens:
        ...
        user_query = g["query"]                    # the question we want answered
        answer = generate(user_query, context)    # RUN the generator -> actual_output

        test_cases.append(
            LLMTestCase(
                input=user_query,
                actual_output=answer,             # the generated answer we're judging
                retrieval_context=context,        # faithfulness checks the generated answer against the golden context
